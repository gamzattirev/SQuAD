...
def create_json(label,num_data):
    ...
    possible_cnt=0
    impossible_cnt = 0
    num_possible = round(num_data * POSSIBLE_RATE)
    num_impossible = num_data - num_possible
    ...
    with open(INPUT_FILE, 'r') as file:
        for row in file:
            ...
            for sentense in sentenses:
                if possible_cnt>num_possible and impossible_cnt>num_impossible:
                    return

                sentense=sentense+SENTENSE_DELIMETER
                ...
                for tool in tools:
                    ...
                    if len(dict_qa['answers']) > 0:
                        is_impossible = 'false'
                        possible_cnt=possible_cnt+1
                    else:
                        impossible_cnt=impossible_cnt+1
                ...
                if (possible_cnt < num_possible and dict_qa['is_impossible'] == 'false') \
                        or (impossible_cnt < num_impossible and dict_qa['is_impossible'] == 'true'):
                    dict['qas'].append(dict_qa)

                if (possible_cnt < num_possible and dict_qa2['is_impossible'] == 'false') \
                            or (impossible_cnt < num_impossible and dict_qa2['is_impossible'] == 'true'):
